...
class pressure_storage:
	...
	if not matched:	
		...
		self.movement = self.movement + value * 100
		if (self.movement>= 1300):
            		self.movement = 1300.0
            	if (self.movement<=850):
            		self.movement = 850.0
...
class matching:
    ...
    def set_pressure(self,target, current):
	...
	if math.fabs(value) <= 0.1: # Error rate
		self.matched = True
	else:
		self.matched = False
	if not self.matched:
...
class gripper:
    ...
    def drawer(self,canvas):
        ...
        canvas.draw_polygon([[self.b[0] - 40 + scale, self.b[1] - 5], [self.b[0] - 70 + scale, self.b[1] - 5], [self.b[0] - 40 + scale, self.b[1] - 50]],3,'Silver')
        canvas.draw_polygon([[self.b[0] + 40 - scale, self.b[1] - 5], [self.b[0] + 70 - scale, self.b[1] - 5], [self.b[0] + 40 - scale, self.b[1] - 50]],3,'Silver')
        canvas.draw_circle([self.b[0] + 20 * math.sin(self.current_val * 2 * math.pi),self.b[1]+20+20 * math.cos(self.current_val *2 *math.pi)], 2, 3, 'Blue')
        canvas.draw_circle([self.b[0] - 20 * math.sin(self.current_val * 2 * math.pi),self.b[1]+20-20 * math.cos(self.current_val *2 *math.pi)], 2, 3, 'Green')
...
def pass_pressure():
    ...
    match.set_pressure(haptic.get_pressure_val(), myfile.get_pressure_val())
    movement = haptic_gripper.get_current() - cyton_gripper.get_current()
    if abs(movement) <= 0.01: # Sync Error Rate
	movement = 0
    haptic.write_val(False,movement)
...
#Defination for different classes
cyton_gripper = gripper([400, 400], [-1,1])
haptic_gripper = gripper([400, 500], [1300,850])
