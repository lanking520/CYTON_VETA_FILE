...
s = socket.socket( socket.AF_INET, socket.SOCK_DGRAM)#Create UDP Socket module
...
class packet:
	def __init__(self):
		...
		self.source_info = ('192.168.0.1',12345) 
	...
	def drawer(self, canvas):
		...
		canvas.draw_text('Connection from IP: ' + self.source_info[0], (300, 460), 20, 'White')
		canvas.draw_text('IP Port: ' + str(self.source_info[1]), (300, 480), 20, 'White') 
	...
	def joystick(self):
			...
			self.source_info = temp[1]
			#print self.source_info
        		self.out = list(eval(temp[0])) # stop here waiting for packet
